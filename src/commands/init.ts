...
function scanForCredentials(): ScanHit[] {
  const hits: ScanHit[] = [];
...
async function runSignup(): Promise<boolean> {
  console.log();
  ...
  if (result.data) {
    ...
    if (sessionToken) setConfig('sessionToken', sessionToken);
    if (data.org?.id) setConfig('orgId', data.org.id);
    if (data.org?.name) setConfig('orgName', data.org.name);
    if (data.user?.id) setConfig('userId', data.user.id);
    if (data.user?.name) setConfig('userName', data.user.name);
    ...
    .action(async (options) => {
      ...
      if (policyMethod === 'ai') {
        ...
        if (genResult.error) {
          ...
          const preview = policyContent.split('\n').slice(0, 10).map((l: string) => `  ${output.dim(l)}`).join('\n');
          console.log(preview);
          if (policyContent.split('\n').length > 10) console.log(output.dim('  ...'));
          console.log();
        } else {
